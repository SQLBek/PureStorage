##############################################################################################################################
# Refresh VMDK with Snapshot Demo
#
...
#
# IMPORTANT:
# To utilize this, you must pre-setup the target VM with a cloned datastore from the source already.  You will only be 
# utilizing specific VMDKs out of that cloned datastore - others can be ignored since they are deduped on FlashArray.
#
# One can use subsets of the code below to clone the originating datastore, present it to the target server, then
# attach the VMDK(s) of interest, then attach the SQL database files.  After, one can then use this script fully
# to refresh the data files in the cloned datastore that is attached to the target server.
#
...
$TargetDevices = @('6000c29d8fbd57d2c64b173eb9a6f3bd')                # Target Disk Serial Number(s)
$VIServerName = 'vc01.fsa.lab'
$ClusterName = 'Workload Cluster 1'
$SourceDatastoreName = 'ayun_sql_ds1'
$SourceVMDKPath = "ayun-sql19-03_1/ayun-sql19-03.vmdk"
$ArrayName = 'sn1-c60-e12-16.puretec.purestorage.com'
$SourceVolumeName = 'ayun_sql_ds1'
$TargetVolumeName = 'ayun_sql_ds2'
